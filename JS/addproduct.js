...
$(document).ready(function() {
    ...
    async function getProductData(productId) {
        try {
            const response = await fetch(`./API/getdata.php?id=${encodeURIComponent(productId)}`);
            if (!response.ok) {
...
async function deleteProduct(productId) {
    try {
        const response = await fetch(`./API/delete_product.php?id=${encodeURIComponent(productId)}`, {
            method: 'DELETE'
        });
        if (!response.ok) {
